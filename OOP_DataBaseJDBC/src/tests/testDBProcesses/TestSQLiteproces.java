...
public class TestSQLiteproces {

	private UserUnit user = new UserUnit(-1, "Mr", "Bond", "stg@gmail.com", new Date());
	private DBSources dbsource = DBSources.SQLiteDB;
	
	@Test
	public void testSQLite_access() {
	...
	@Test
	public void testSQLite_read() {
		CollectiveDBManager sqliteman = new CollectiveDBManager();
		if(!sqliteman.connectDB(dbsource))
			Assert.fail("No db connection");
		List<UserUnit> list = sqliteman.loadInUsers();
		
		if(list == null)
			Assert.fail("No data arrived");
		if(!list.get(0).getFname().equals("some") || !list.get(0).getLname().equals("body"))
			Assert.fail("No proper content arrived " + list.get(0).toString());
	...
	@Test
	public void testSQLite_CUDdatas() {
		CollectiveDBManager sqliteman = new CollectiveDBManager();
		if(!sqliteman.connectDB(dbsource))
			Assert.fail("No db connection");
